...
class ChemModel(object):
    ...
    def load_data(self, file_name, is_training_data: bool):
        ...
        for g in data:
            self.max_num_vertices = max(self.max_num_vertices, max(
                [v for e in g['graph'] for v in [e[0], e[2]]]))
        ...
        for (internal_id, task_id) in enumerate(self.params['task_ids']):
            with tf.variable_scope("out_layer_task%i" % task_id):
                ...
                computed_values = self.gated_regression(self.ops['final_node_representations'],
                                                        self.weights['regression_gate_task%i' %
                                                                     task_id],
                                                        self.weights['regression_transform_task%i' % task_id])
                diff = computed_values - \
                    self.placeholders['target_values'][internal_id, :]
    ...
    def make_train_step(self):
        ...
        if self.args.get('--freeze-graph-model'):
            graph_vars = set(self.sess.graph.get_collection(
    ...
    def run_epoch(self, epoch_name: str, data, is_training: bool):
        ...
        loss = 0
        accuracies = []
        accuracy_ops = [self.ops['accuracy_task%i' % task_id]
        ...
        for step, batch_data in enumerate(batch_iterator):
            ...
            if is_training:
                ...
                fetch_list = [self.ops['loss'],
                              accuracy_ops, self.ops['train_step']]
            else:
        ...
        error_ratios = accuracies / \
            chemical_accuracies[self.params["task_ids"]]
    ...
    def train(self):
        ...
        with self.graph.as_default():
            if self.args.get('--restore') is not None:
                ...
                print(
                    "\r\x1b[KResumed operation, initial cum. val. acc: %.5f" % best_val_acc)
            else:
            ...
            for epoch in range(1, self.params['num_epochs'] + 1):
                ...
                accs_str = " ".join(["%i:%.5f" % (id, acc) for (
                    id, acc) in zip(self.params['task_ids'], train_accs)])
                errs_str = " ".join(["%i:%.5f" % (id, err) for (
                    id, err) in zip(self.params['task_ids'], train_errs)])
                ...
                accs_str = " ".join(["%i:%.5f" % (id, acc) for (
                    id, acc) in zip(self.params['task_ids'], valid_accs)])
                errs_str = " ".join(["%i:%.5f" % (id, err) for (
                    id, err) in zip(self.params['task_ids'], valid_errs)])
            ...
            for variable in self.sess.graph.get_collection(tf.GraphKeys.GLOBAL_VARIABLES):
                ...
                if variable.name in data_to_load['weights']:
                    restore_ops.append(variable.assign(
                        data_to_load['weights'][variable.name]))
                else:
                    print(
