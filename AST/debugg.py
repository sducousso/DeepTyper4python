...
class FBetaMeasure(Metric):
    ...
    def __call__(self,
        ...
        if true_positives_bins.shape[0] == 0:
            ...
            true_positive_sum = torch.bincount(
                true_positives_bins.long(), minlength=num_classes).float()
        ...
        if gold_labels.shape[0] != 0:
            true_sum = torch.bincount(
                gold_labels_bins, minlength=num_classes).float()
        else:
    ...
    def _true_negative_sum(self):
        if self._total_sum is None:
            ...
            true_negative_sum = self._total_sum - self._pred_sum - \
                self._true_sum + self._true_positive_sum
