...
class SequencedGraphEncoder:
    def __init__(self,
        ...
        if encoder_type == "bidirectional_rnn" and num_units % 2 != 0:
            raise ValueError(
                "num_units must be a multiple of two when using bidirectional rnns")
    ...
    def __call__(self,
        ...
        if self.encoder_type == "bidirectional_rnn":
            rnn_path_representations, rnn_state = tf.nn.bidirectional_dynamic_rnn(
                ...
                dtype=tf.float32,
                swap_memory=True)
            rnn_path_representations = tf.concat(
                rnn_path_representations, axis=-1)
            ...
            rnn_state = tf.contrib.framework.nest.pack_sequence_as(
                rnn_state[0], f_rnn_state)
        ...
        shifted_paths = shifted_paths * \
            tf.sequence_mask(primary_path_lengths, dtype=tf.int64)
        ...
        output = self.output_map(
            tf.concat([rnn_representations, node_representations], axis=-1))
        ...
        self.state_map = tf.layers.Dense(
            ...
            use_bias=False,
            kernel_initializer=eye_glorot)
        flatten_state = self.state_map(
            tf.concat([flatten_rnn_state, graph_state], axis=-1))

        # defatten
        flatten_state = tf.split(flatten_state, state_sizes, axis=1)
        state = tf.contrib.framework.nest.pack_sequence_as(
            rnn_state, flatten_state)
        return output, state
